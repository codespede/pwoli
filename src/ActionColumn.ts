import { Application as Pwoli, Column, DataHelper, Html } from ".";
    ...
    protected initDefaultButton(name, iconName, additionalOptions = {}) {
        if (this.buttons[name] === undefined && this.template.indexOf(`{${name}}`) !== -1) {
            this.buttons[name] = (url, model, key) => {
                ...
                let options = { title, 'aria-label': title, 'data-pjax': '0', ...additionalOptions, ...this.buttonOptions };
                let icon = this.icons[iconName] !== undefined ? this.icons[iconName] : Html.tag('span', '', { class: `glyphicon glyphicon-${iconName}` });
    ...
    protected async renderDataCellContent(model, key, index) {
        return await DataHelper.replaceAsync(this.template, /{\w+}/g, async (match) => {
            ...
            if (this.visibleButtons[name] !== undefined)
                isVisible = typeof this.visibleButtons[name] === 'function' ? (this.visibleButtons as {})[name](this.visibleButtons[name], model, key, index) : this.visibleButtons[name];
            else
                isVisible = true;
