...
export default class ActiveForm {
    model;
    errorClass = 'error';
    fieldTemplate = '<label for="{fieldName}" class="input-label">{fieldLabel}</label><br><[type] [htmlOptions] type="text" placeholder="{placeholder}" name="{fieldName}" value="{fieldValue}" class="{hasError}">{errorHtml}';
    constructor(model) {
        this.model = model;
    }
    public renderField(name, type = 'input', htmlOptions = {}) {
        const html = this.fieldTemplate.replace(/{\w+}/g, (match) => { return this.renderSection(match, name); });
        const options = [];
        for (const p in htmlOptions)
            if (htmlOptions.hasOwnProperty(p))
                options.push(p + '="' + htmlOptions[p] + '"');
        htmlOptions = options.join(' ');
        const namespaces = { type, htmlOptions };
        return html.replace(/\[\w+\]/g, (match) => { return namespaces[match.replace(/\[|\]/g, '')]; })
    }
    ...
    public sections = {
        ...
        errorHtml: (name) => {
            return this.model.errors.hasOwnProperty(name)? '<span class="badge"></span><p class="error-message">' + this.model.errors[name] + '</p>' : '';
        },
        hasError: (name) => {
