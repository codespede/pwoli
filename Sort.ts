...
export default class Sort extends Component {
    ...
    public link(attribute, options) {
        ...
        if (options.label !== undefined) {
            ...
            label = this.attributes[attribute].label !== undefined ? this.attributes[attribute].label : Inflector.humanize(attribute)
        // let optionsHtml;
    ...
    public createUrl(attribute, absolute = false) {
        ...
        for (const param in params)
            sortUrl.searchParams.set(param, params[param]);
        return sortUrl;
    }

    public createSortParam(attribute) {
        if (this.attributes[attribute] === undefined)
            throw new exception(`Unknown sort attribute: ${attribute}`);
        ...
        let i = 0;
        if (attribute === 'title')
            console.log('csp', this.attributes[attribute], direction, directions);
        ...
        if (direction !== false) {
            ...
            direction = definition.default !== undefined ? definition.default : 'asc';
        directions = this.enableMultiSort ? [[attribute, direction]].push(directions) : [[attribute, direction]];

        const sorts = [];
        for (const dir of directions)
            if(dir[0] !== undefined)
                sorts.push(dir[1] === 'desc' ? `-${dir[0]}` : dir[0]);
