...
export default class View extends Component {
    ...
    public async publishAndRegisterFile(file, position = 'end') {
        ...
        fs.copyFile(file, `${this.basePath}/${path.basename(file)}`, (error) => { if (error) throw error; });
        console.log('parf', file, path.extname(file))
        this.registerFile(path.extname(file) === '.js' ? 'js' : 'css', `${this.basePath}/${path.basename(file)}`, { position });
    }
    ...
    public renderHeadHtml() {
        ...
        if (this.cssFiles.length > 0)
            lines.push(this.cssFiles.join("\n"));
        if (this.jsFiles.head.length > 0)
            lines.push(this.jsFiles.head.join("\n"));
    ...
    public renderEndHtml(ajaxMode) {
        ...
        if (this.jsFiles.end.length > 0)
            lines.push(this.jsFiles.end.join("\n"));
        if (ajaxMode) {
            const scripts = [];
            if (this.js.end.length > 0)
                scripts.push(this.js.end.join("\n"));
            if (this.js.ready.length > 0)
                scripts.push(this.js.ready.join("\n"));
            if (this.js.load.length > 0)
                scripts.push(this.js.load.join("\n"));
            if (scripts.length > 0)
                lines.push(Html.script(scripts.join("\n")));
        } else {
            let js;
            if (this.js.end.length > 0){
                js = this.js.end.join("\n");
    ...
    protected findViewFile(view) {
        let file;
        if (view.indexOf('@') === 0)
            file = Application.getAlias(view);
        else if (view.indexOf('/') === 0)
            file = Application.getViewPath() + view;
         else
            throw new exception(`Unable to resolve view file: ${view}`);
