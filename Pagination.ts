...
export default class Pagination extends Component {
    ...
    public getPage(recalculate = false){
        if (this._page === undefined || recalculate)
            this.setPage(this.getQueryParam(this.pageParam, 1) - 1, true);
    ...
    public getPageSize() {
        if (this._pageSize === undefined) {
            if (this.pageSizeLimit.length === 0 || (this.pageSizeLimit[0] === undefined || this.pageSizeLimit[1] === undefined))
                ...
                this.setPageSize(this.getQueryParam(this.pageSizeParam, this.defaultPageSize), true);
        }
    ...
    public createUrl(page, pageSize, absolute = false) {
        ...
        if (pageSize !== undefined && pageSize !== this.defaultPageSize)
    ...
    public getLinks(absolute = false){
        ...
        if (pageCount > 0) {
            ...
            if (currentPage > 0)
                links.prev = this.createUrl(currentPage - 1, null, absolute)
    ...
    protected getQueryParam(name, defaultValue){
        ...
        return (params[name] !== undefined && (/boolean|number|string/).test(typeof params[name])) ? params[name] : defaultValue;
    }
}
