...
mixing [EJS](https://ejs.co/) and HTML in a single file, but when it is
generated dynamically it starts to get tricky to handle it without extra help. Pwoli provides such help in the form
...
Since building dynamic HTML by string concatenation can get messy very fast, Pwoli provides a set of methods to
...
-   The values of attributes will be HTML-encoded using [[Html.encode|Html.encode]].
-   If the value of an attribute is an array, it will be handled as follows:

    -   If the attribute is a data attribute as listed in [[Html.dataAttributes]], such as `data` or `ng`,
        a list of attributes will be rendered, one for each element in the value array. For example,
        `{data: {id: 1, name: 'pwoli'}}` generates `data.id="1" data.name="pwoli"`; and
        `{data: {params: {id: 1, name: 'pwoli'},        status: 'ok'}` generates
        `data.params={"id":1,"name":"pwoli"} data.status="ok"`. Note that in the latter example JSON format is used
        to render a sub-array.
    -   If the attribute is NOT a data attribute, the value will be JSON-encoded. For example,
        `{params: {id: 1, 'name: 'pwoli'}` generates `params={"id":1,"name":"pwoli"}`.
...
```js
let let options = {class: 'btn btn-default'};
...
console.log(Html.tag('div', 'Pwede na', options));
...
console.log(Html.tag('div', 'Save', options));
...
```js
let options = {class: 'btn btn-default'};
...
```js
let options = {style: {width: '100px', height: '100px'}};
...
When using [[Html.addCssStyle|addCssStyle]], you can specify either an array of key-value pairs,
corresponding to CSS property names and values, or a string such as `width: 100px; height: 200px;`. These formats
can be converted from one to the other using [[Html.cssStyleFromArray|cssStyleFromArray]] and
[[Html.cssStyleToArray|cssStyleToArray]]. The [[Html.removeCssStyle|removeCssStyle]]
...
For this, Pwoli provides two compact and simple-to-use methods:

```js
let userName = Html.encode(user.name);
console.log(userName);

let decodedUserName = Html.decode(userName);
```
...
> Note: consider using [[ActiveForm|ActiveForm]] in case you're dealing with models and need validation.

### Creating Forms <span id="creating-forms"></span>

Forms can be opened with [[Html.beginForm|beginForm]] method like the following:

```js
<%- Html.beginForm(['order/update', {id: id}, 'post', {enctype: 'multipart/form-data'}]) %>
```

The first argument is the URL the form will be submitted to. It can be specified in the form of a Pwoli route.
...
The title isn't encoded, so if you're displaying data from the end user, encode it with [[Html.encode|Html.encode]].
...
-   [[Html.submitInput]]
-   [[Html.resetInput]]
-   [[Html.textInput]], [[Html.activeTextInput]]
-   [[Html.hiddenInput]], [[Html.activeHiddenInput]]
-   [[Html.passwordInput]] / [[Html.activePasswordInput]]
-   [[Html.fileInput]], [[Html.activeFileInput]]
-   [[Html.textarea]], [[Html.activeTextarea]]
...
```js
// Post[title]
console.log(Html.getInputName(post, 'title'));

// post-title
console.log(Html.getInputId(post, 'title'));

// my first post
console.log(Html.getAttributeValue(post, 'title'));

// post->authors[0]
console.log(Html.getAttributeValue(post, '[0]authors[0]'));
```
...
```js
// dates
console.log Html.getAttributeName('dates[0]');
```
...
```js
<%- Html.cssFile('css/ie5.css', {condition: 'IE 5'}) %>
...
```js
<%- Html.a('Profile', `user/view/${id}`, {class: 'profile-link'}) %>
```
...
```js
<%- Html.img('images/logo.png', {alt: 'My logo'}) %>
...
Besides [aliases](concept-aliases.md), the first argument can accept routes, parameters and URLs.
