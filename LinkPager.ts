...
export default class LinkPager extends Widget {
    ...
    protected async renderPageButtons() {
        ...
        if (this.prevPageLabel !== false) {
            ...
            if (page < 0)
                page = 0;
            buttons.push(await this.renderPageButton(this.prevPageLabel, page, this.prevPageCssClass, currentPage <= 0, false));
        }

        // internal pages
        const [beginPage, endPage] = this.getPageRange();
        for (let i = beginPage; i <= endPage; i++){
            buttons.push(await this.renderPageButton(i + 1, i, undefined, this.disableCurrentPageButton && i === currentPage, i === currentPage));
        }
        ...
        if (this.nextPageLabel !== false) {
            ...
            if (page >= pageCount - 1)
                page = pageCount - 1;
            buttons.push(await this.renderPageButton(this.nextPageLabel, page, this.nextPageCssClass, currentPage >= pageCount - 1, false));
        }

        // last page
        const lastPageLabel = this.lastPageLabel === true ? pageCount : this.lastPageLabel;
        if (lastPageLabel !== false)
            buttons.push(await this.renderPageButton(lastPageLabel, pageCount - 1, this.lastPageCssClass, currentPage >= pageCount - 1, false));
