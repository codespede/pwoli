...
export default class CollectionView extends Widget{
    ...
    public async run() {
        ...
        if (this.showOnEmpty || this.dataProvider.getCount() > 0)
            content = await DataHelper.replaceAsync(this.layout, /{\w+}/g, (async (match) => { console.log('run-match', match, this.dataProvider.totalCountPromise); return await this.renderSection(match); }))
        else
