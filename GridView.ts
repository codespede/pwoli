...
export default class GridView extends CollectionView {
    ...
    protected getClientOptions() {
        ...
        if (this.filterSelector !== undefined)
    ...
    public async renderItems() {
        ...
        if (this.showFooter)
            if (this.placeFooterAfterBody)
                tableFooterAfterBody = await this.renderTableFooter();
            else
                tableFooter = await this.renderTableFooter();
        const content = [caption, columnGroup, tableHeader, tableFooter, tableBody, tableFooterAfterBody].filter(item => item);
        ...
        for (const column of this.columns) {
            if (column.options !== undefined && column.options.length > 0) {
                ...
                for (const col of this.columns)
                    cols.push(Html.tag('col', '', col.options))
    ...
    public async renderTableHeader() {
        const cells = [];
        for (const column of this.columns)
            cells.push(await column.renderHeaderCell());
        let content = Html.tag('tr', cells.join(''), this.headerRowOptions);
        if (this.filterPosition === 'header')
            content = await this.renderFilters() + content;
        else if (this.filterPosition === 'body')
            content = content + await this.renderFilters();
    ...
    public async renderTableFooter() {
        const cells = [];
        for (const column of this.columns)
            cells.push(await column.renderFooterCell());
        let content = Html.tag('tr', cells.join(''), this.footerRowOptions);
        if (this.filterPosition === 'footer')
            content = await this.renderFilters();
        return `<tfoot>\n${content}\n</tfoot>`;
    }
    ...
    public async renderFilters() {
        if (this.filterModel !== undefined) {
            const cells = [];
            for (const column of this.columns)
                cells.push(await column.renderFilterCell());
    ...
    public async renderTableRow(model, key, index) {
        const cells = [];
        for (const column of this.columns)
            cells.push(column.renderDataCell(model, key, index));
    ...
    public async initColumns() {
        ...
        for (let column of this.columns) {
            if (typeof column === 'string')
                ...
                column = new this.dataColumnClass({ grid: this, ...column });
            if (!column.visible) {
    ...
    protected guessColumns() {
        ...
        for (const name in model)
            if (model[name] === null || (/boolean|number|string/).test(typeof model[name]))
